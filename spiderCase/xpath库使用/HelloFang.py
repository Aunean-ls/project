...
def crawl(url):
    ...
    for li in li_list:
        ...
        if url_detail:
            ...
            detail_response = requests.get(url=url_detail, headers=headers).content.decode('utf-8')
            ...
            detail_response2 = requests.get(url=url_detail2, headers=headers)
            detail_response2.encoding = 'utf-8'
            detail_response2 = detail_response2.text
            ...
            # 装修状况
            decoration_status = sel2.xpath('//div[@class="main-item"][1]/ul/li[2]/div[2]/li[2]/div[2]/text()')[
                0].strip()
...
page = int(input("请输入结束页："))

if __name__ == '__main__':
    url_list = ['https://cs.newhouse.fang.com/house/s/b9{}/'.format(i) for i in range(1, page)]
    for url in url_list:
        crawl(url)
